import { pgTable, uuid, varchar, timestamp, foreignKey, unique, serial, text, jsonb, numeric, boolean, check, integer, primaryKey, pgEnum } from "drizzle-orm/pg-core"
...
export const itemHierarchy = pgTable("item_hierarchy", {
	...
	check("no_self_reference", sql`parent_item_id <> child_item_id`),
]);
...
export const itemTransactions = pgTable("item_transactions", {
	...
	check("valid_stock_movement", sql`((movement = 'IN'::movement_type) AND (new_stock = (prev_stock + movement_amt))) OR ((movement = 'OUT'::movement_type) AND (new_stock = (prev_stock - movement_amt)))`),
]);
...
export const vendorServices = pgTable("vendor_services", {
	...
	check("vendor_services_rate_check", sql`rate >= 0`),
]);
...
export const vendorTransactions = pgTable("vendor_transactions", {
	...
	check("vendor_transactions_rate_check", sql`rate >= 0`),
	check("vendor_transactions_quantity_check", sql`quantity > 0`),
]);

export const vendorLedger = pgTable("vendor_ledger", {
	ledgerId: uuid("ledger_id").defaultRandom().primaryKey().notNull(),
	vendorId: uuid("vendor_id").notNull(),
	transactionDate: timestamp("transaction_date", { withTimezone: true, mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	...
	check("vendor_ledger_transaction_type_check", sql`(transaction_type)::text = ANY ((ARRAY['PAYMENT'::character varying, 'INVOICE'::character varying, 'ADJUSTMENT'::character varying, 'STARTING_BALANCE'::character varying])::text[])`),
	check("vendor_ledger_check", sql`(debit >= (0)::numeric) AND (credit >= (0)::numeric)`),
...
export const vendorPayment = pgTable("vendor_payment", {
	...
	check("vendor_payment_amount_check", sql`amount > (0)::numeric`),
]);
