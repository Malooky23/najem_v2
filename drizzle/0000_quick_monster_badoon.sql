...
CREATE TABLE "inventory" (
	...
	CONSTRAINT "inventory_quantity_check" CHECK (quantity
  >= 0)
);
--> statement-breakpoint
CREATE TABLE "item" (
...
CREATE TABLE "item_hierarchy" (
	...
	"created_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
	"updated_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
	CONSTRAINT "no_self_reference" CHECK (parent_item_id
  <> child_item_id)
);
--> statement-breakpoint
...
CREATE TABLE "item_transactions" (
	...
	"created_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
	"updated_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
	CONSTRAINT "valid_stock_movement" CHECK (((movement = 'IN'::movement_type) AND (new_stock = (prev_stock + movement_amt)))
...
CREATE TABLE "order_items" (
	...
	"created_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
	"updated_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
	CONSTRAINT "order_items_quantity_check" CHECK (quantity
  > 0)
);
--> statement-breakpoint
CREATE TABLE "orders" (
...
CREATE TABLE "vendor_ledger" (
	...
	"created_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
	"updated_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
	CONSTRAINT "vendor_ledger_transaction_type_check" CHECK ((transaction_type)
	...
	CONSTRAINT "vendor_ledger_check" CHECK ((debit >= (0):: numeric)
...
CREATE TABLE "vendor_payment" (
	...
	"created_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
	"updated_at" timestamp with time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
	CONSTRAINT "vendor_payment_amount_check" CHECK (amount
  > (0)::numeric)
);
--> statement-breakpoint
CREATE TABLE "vendor_services" (
	...
	CONSTRAINT "vendor_services_rate_check" CHECK (rate
  >= 0)
);
--> statement-breakpoint
CREATE TABLE "vendor_transactions" (
	...
	CONSTRAINT "vendor_transactions_rate_check" CHECK (rate
  >= 0),
	CONSTRAINT "vendor_transactions_quantity_check" CHECK (quantity
  > 0)
);
--> statement-breakpoint
...
ALTER TABLE "item_owners" ADD CONSTRAINT "item_owners_item_id_fkey" FOREIGN KEY ("item_id") REFERENCES "public"."item"("item_id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "vendor_transactions" ADD CONSTRAINT "vendor_transactions_emp_id_fkey" FOREIGN KEY ("emp_id") REFERENCES "public"."employee"("emp_id") ON DELETE restrict ON UPDATE no action;